...
```python
strict = client.guardrail_policies.create(
    name="Strict — Billing",
    config={
...
# it's not a parameter on that consumer's own create()/update() call.
client.guardrail_policies.attach(strict.id, "agent", copilot.id)
...
# request would hit.
check = client.guardrail_policies.test(
    policy_id=strict.id,
    stage="pre_llm",
    content="My email is jane@example.com, please refund my order.",
)
print(check.passed, check.content)  # False, redacted content

# Check-volume analytics and version history come with every policy.
stats = client.guardrail_policies.analytics(strict.id, days=30)
versions = client.guardrail_policies.versions.list(strict.id)
```
...
v2 = client.prompts.versions.create(
    prompt.id,
...
client.prompts.publish(prompt.id, version_id=v2.id)
